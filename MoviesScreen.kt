...
@Composable
fun MoviesScreen(repo: ContentRepository = remember { ContentRepository() }) {
    var items by remember { mutableStateOf<List<CardItem>>(emptyList()) }
    LaunchedEffect(Unit) {
        runCatching { repo.movies() }
...
private fun PaddingValuesCompat() = androidx.compose.foundation.layout.PaddingValues(16.dp)
