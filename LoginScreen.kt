...
fun LoginScreen(viewModel: AuthViewModel) {
    ...
    Column(modifier = Modifier.fillMaxWidth().padding(24.dp)) {
        ...
        TextButton(onClick = { viewModel.switchMode(AccountScreenMode.FORGOT_PASSWORD) }) {
            Text(stringResource(R.string.forgot_password), color = NeonPurple)
        }
        ...
        Spacer(Modifier.height(8.dp))
        TextButton(
            onClick = { viewModel.switchMode(AccountScreenMode.REGISTER) },
            modifier = Modifier.fillMaxWidth()
        ) {
            Text(stringResource(R.string.no_account_yet), color = TextSecondary)
        }
